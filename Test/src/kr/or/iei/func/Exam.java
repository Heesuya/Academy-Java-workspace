package kr.or.iei.func;

import kr.or.iei.vo.User;

public class Exam {
	// 생성자, 전역 변수
	User[] userArr;
	int index;

	public Exam() {
		super();
		userArr = new User[10];
		index = 0;

		User user1 = new User("user01", "1234", "유저1", 23, "01022478875");

		User user2 = new User("user02", "1357", "유저2", 27, "01095887774");

		User user3 = new User("user03", "2468", "유저3", 34, "01036425456");

		userArr[index++] = user1;

		userArr[index++] = user2;

		userArr[index++] = user3;
	}

	public void exam1() {
		// 전역 변수는 생성자에서 초기화 값을 넣어준다.
//		User[] userArr = new User[10];
//
//		int index = 0;
//
//		User user1 = new User("user01", "1234", "유저1", 23, "01022478875");
//
//		User user2 = new User("user02", "1357", "유저2", 27, "01095887774");
//
//		User user3 = new User("user03", "2468", "유저3", 34, "01036425456");
//
//		userArr[index++] = user1;
//
//		userArr[index++] = user2;
//
//		userArr[index++] = user3;

		// 비밀번호는 나오면 안된다.
		
		System.out.println("아이디\t이름\t나이\t전화번호");

		// userArr.length로 하게 되면 error, index의 길이를 조율해야 한다.
		for (int i = 0; i < index; i++) {
			// 객체의 private 로 되어있어서 User 타입으로 바꿔서 정보를 가져와야한다.
			User u = userArr[i];
			System.out.println(u.getId() + "\t" + u.getName() + "\t" + u.getAge() + "\t" + u.getPhone());
		}

	}
}

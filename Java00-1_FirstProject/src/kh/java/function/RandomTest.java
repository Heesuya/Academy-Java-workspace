...
//이클립스 console 반복 되지 않게 종료 해놓아야 함. 
	...
	public void nansu() {
		...
		while (power) {//게임 시작 메인 메뉴 
			...
			switch (select) {
			case 1 : //업앤다운 게임 실행 
				int win = 0;//승리 누적 변수 case 1에서 받은 누적된 승리 값 case2에서 사
				int count = 0;//입력된 회차 번호 
			...
			case 2: // 베라31 게임 실행 
				...
				while (baskinRobbinsPower) { //베라 게임 메뉴 
					...
					case 1://베라 게임 시작 
						...
						boolean baskinPower = true; // false => 베라 메뉴 이동 
						while (baskinPower) {
							System.out.print("Input Number : ");
							int inputNum = sc.nextInt();//사용자가 적는 번호 

							if (inputNum == 1 || inputNum == 2 || inputNum == 3) {  //조건문을 통해서 사용자에게 받은 변수에 따라 baskin의 수가 누적되게 한다.
								...
								for (int j = 0; j < com; j++) { //for 반복문 _ 컴퓨터에게 입력받은 임의의 수 출력 
									countBaskin++;
									System.out.println(countBaskin + "!");
